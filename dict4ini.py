...
# Updates:
# custom version--------------
#    Victor Stinner patch: add format constructor option to choose line format
# 0.6-------------------------
#   2006/01/18
...
class DictIni(DictNode):
    def __init__(self, inifile=None, values=None, encoding=None, commentdelimeter='#', sectiondelimeter=section_delimeter, onelevel=False, format="%s=%s"):
        self._items = {}
        ...
        if values:
            ...
            for key, value in self.ordereditems(values, sec=section):
                if isinstance(value, dict):
                    ...
                    default.append(self._format % (key, uni_prt(value, encoding)))
            if default:
                buf.insert(0, '\n'.join(default))
